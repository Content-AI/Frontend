...
const EditBrandVoice = (props) => {

    let navigate = useNavigate();
    const [brandData,setbrandData]=useState(null)
    
    const [showTextInputBox,setshowTextInputBox]=useState(true)
    ...
    const get_voice_data = async() =>{
        const resp = await fetchData(BACKEND_URL+BACK_END_API_BRAND_VOICE+props.ID+"/",props.AUTH_TOKEN)
        if(resp.status==200){
                ...
                <div className="flex justify-between">
                    ...
                    <>
                        <div className="flex items-center justify-center flex-col m-4 p-3">
                            <p className='text-[20px]  font-bold'>{response.content_summarize}</p>
                        </div>
                    </>                     
                }
                    {analyzingdata
