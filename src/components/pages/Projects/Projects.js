...
const Projects = (props) => {
  ...
  const update_folder_name = async(id) => {
    ...
    const resp = await patchData(formData,BACKEND_URL+BACK_END_API_PROJECTS+id+"/",props.AUTH_TOKEN)
    if(resp.status=201){
      get_folder_of_user()
  ...
  const _update_project_data = async (data, id, message) => {
    const resp = await patchData(data, BACKEND_URL + BACK_END_API_PROJECTS + id + "/", props.AUTH_TOKEN)
    if (resp.status == 201) {
  ...
  const create_folder = async() => {
    ...
    const resp = await postData(formData,BACKEND_URL+BACK_END_API_PROJECTS,props.AUTH_TOKEN)
    if(resp.status==201){
  ...
  return (
    <div className="" >
      ...
      <div className="mt-8">
        <div className="title-header flex items-center mb-6">
          ...
          <div className="ml-auto">
            ...
            className="text-md cursor-pointer">See more templates</button>
          </div>
        </div>
        <div>
          <div className="cardwrap grid grid-cols-3 xl:grid-cols-3 gap-x-6 gap-y-5">
            <div className="cursor-pointer hover:border hover:shadow-lg focus:ring-gray-400 focus:shadow-xl duration-150  hover:ring-gray-300 hover:ring-2 flex flex-col items-center justify-center text-white bg-blue  rounded-xl"
              onClick={()=>{
        ...
        ?
          ...
          <div className="mb-8 flex w-full max-w-full flex-wrap sm:-mr-6">
            {FolderOfUser.map((data,index)=>{
              return (
                      <div className="flex w-[250px] cursor-pointer items-start pb-6 mr-1" key={index}>
                        ...
                        <div className="  bg-[#fafbfd]  shadow-lg relative top-0 flex max-w-full flex-1 items-center rounded-xl border border-purple-100  py-1 pl-2 transition-top duration-200 hover:-top-1 md:mr-6">
                            <div className="w-full grid grid-rows-1">
                              ...
                              <div className="tooltip-container  pr-4"
                               onClick={()=>{
                                  navigate(`/folder_of_user/${data.id}?folder_name=${data.project_name}`)
                                }}>

                              <div className="flex">
                                <div>
                                    <svg className="w-5 h-5 m-2" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                      <path opacity="0.5" d="M22 14V11.7979C22 9.16554 22 7.84935 21.2305 6.99383C21.1598 6.91514 21.0849 6.84024 21.0062 6.76946C20.1506 6 18.8345 6 16.2021 6H15.8284C14.6747 6 14.0979 6 13.5604 5.84678C13.2651 5.7626 12.9804 5.64471 12.7121 5.49543C12.2237 5.22367 11.8158 4.81578 11 4L10.4497 3.44975C10.1763 3.17633 10.0396 3.03961 9.89594 2.92051C9.27652 2.40704 8.51665 2.09229 7.71557 2.01738C7.52976 2 7.33642 2 6.94975 2C6.06722 2 5.62595 2 5.25839 2.06935C3.64031 2.37464 2.37464 3.64031 2.06935 5.25839C2 5.62595 2 6.06722 2 6.94975V14C2 17.7712 2 19.6569 3.17157 20.8284C4.34315 22 6.22876 22 10 22H14C17.7712 22 19.6569 22 20.8284 20.8284C22 19.6569 22 17.7712 22 14Z" fill="#1C274C"/>
                                      <path d="M12.25 10C12.25 9.58579 12.5858 9.25 13 9.25H18C18.4142 9.25 18.75 9.58579 18.75 10C18.75 10.4142 18.4142 10.75 18 10.75H13C12.5858 10.75 12.25 10.4142 12.25 10Z" fill="#1C274C"/>
                                    </svg>
                                </div>
                                <div>
                                    <input type="text" className="flex flex-1 p-2 outline-none max-w-full rounded-md border-none text-sm cursor-pointer focus:ring-0 group-hover:text-blue-800 min-w-full overflow-ellipsis whitespace-nowrap pr-7" readOnly="" spellCheck="false" required="" data-testid="project-folder-name" data-hj-allow="true" value={data.project_name}
                                      readOnly
                                    />
                                </div>
                              </div>
                              ...
                              <div className="grid content-center w-full ml-2">
                                ...
                                {data.documents_data}</span>
                              </div>
                              <span className="tooltip-text">Open {data.project_name} folder</span>
</div>

                              
                              <div className="flex flex-1 pl-2">
                                  <div className="grid content-center w-full">
                                    <span className="flex-grow block text-sm text-gray-500 whitespace-nowrap">Created on {data.created_at}</span>
                                  </div>
                                        ...
                                        <div className="items-center gap-2 mt-3 sm:flex">
                                            <button
                                                ...
                                                onClick={() =>{
                                                  ...
                                                  _update_project_data(formData, DeleteFolderId, "Moved to trash")
                                                }}
                                            >
                                                Delete
                                            </button>
                                            ...
                                            <div className="py-1 !mt-0 flex items-center gap-2 bg-white w-full px-3 rounded-lg ring-1 hover:ring-2 transition-all duration-150 ease-in-out ring-gray-200 outline-none focus-within:!ring-1">
                                              <div className="flex items-center grow gap-2 py-1.5">
                                                <div className="flex gap-1 grow">
                                                  <input
                                                    value={RenameFolder} onChange={handleFolderRenameChange}
      ...
      <>
        ...
        <div className="fixed inset-0 overflow-y-auto z-50" id="headlessui-dialog-1749" role="dialog" aria-modal="true" aria-labelledby="headlessui-dialog-title-1753">
          <div className="flex min-h-screen items-center justify-center text-center sm:block sm:p-0">
              <div className="fixed inset-0 bg-grey-500 bg-opacity-75 transition-opacity" id="headlessui-dialog-overlay-1751" aria-hidden="true"></div>
              ...
              <div className="inline-block rounded-lg bg-white align-bottom p-6 sm:p-8 shadow-xl transform text-left transition-all sm:my-8 sm:align-middle sm:max-w-xl sm:w-full undefined">
                <button data-testid="close-modal" className="outline-none absolute top-0 right-0 z-10 p-8 ring-0 hover:opacity-70" tabIndex="0" aria-label="Close"
                onClick={()=>{
                  setPopUpModelToCreateFolder(false)
                  setFolderName(null)
                }}
                >
                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path fillRule="evenodd" clipRule="evenodd" d="M13.7071 3.70711C14.0976 3.31658 14.0976 2.68342 13.7071 2.29289C13.3166 1.90237 12.6834 1.90237 12.2929 2.29289L8 6.58579L3.70711 2.29289C3.31658 1.90237 2.68342 1.90237 2.29289 2.29289C1.90237 2.68342 1.90237 3.31658 2.29289 3.70711L6.58579 8L2.29289 12.2929C1.90237 12.6834 1.90237 13.3166 2.29289 13.7071C2.68342 14.0976 3.31658 14.0976 3.70711 13.7071L8 9.41421L12.2929 13.7071C12.6834 14.0976 13.3166 14.0976 13.7071 13.7071C14.0976 13.3166 14.0976 12.6834 13.7071 12.2929L9.41421 8L13.7071 3.70711Z" fill="currentColor"></path>
                    </svg>
                </button>
                <div className="text-left">
                    <h3 className="pt-8 pr-8 text-xl font-medium text-black sm:pt-0 " id="headlessui-dialog-title-1753">Create Folder</h3>
                    <div>
                          <div className="flex w-full flex-1 flex-col undefined">
                            <div className="mb-3 flex flex-1 flex-col justify-between lg:flex-row"><label htmlFor="folder-name" className="text-sm font-bold text-grey-700 !font-light w-100">Folder Name</label></div>
                            <div className="w-full">
                            <input data-testid="folder-name" placeholder="New Folder" className="flex flex-1 border py-2.5 px-3.5 border-purple-100 focus:outline-none w-full resize-none rounded placeholder-grey-400 shadow-sm focus:border-green-700 focus:ring-0 text-grey-800" type="text" autoComplete="off" id="folder-name" data-hj-allow="true"
                              value={folderName}
                          ...
                          >
                          <span className="flex-nowrap whitespace-nowrap">Create Folder</span>
                          </button>
                          ...
                          >
                            <span className="text-inherit inline-block text-center ring-offset-2 duration-200 focus:ring-2 cursor-pointer hover:opacity-70 relative opacity-100 self-center" tabIndex="0" role="button">
                              Cancel
                            <span className="absolute -bottom-0.5 block w-full border-b border-grey-200">                    
                            </span>
                            </span>
                          </button>
                          </div>
